...
{
    public class AdminReturnBookStep1Model : PageModel
    {
        ...
        public void OnPost()
        {
            ...
            if (bookInfo.InventoryNum.Length == 0)
            {
            ...
            else
            {
                try
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        ...
                        string sql = $"SELECT count(*) FROM [dbo].[Book] where InventoryNum=@inventoryNum and IDLibrary={AdminLibraryInfoModel.libraryInfo.Id} and IsAvaiable='НЕ';";
                        ...
                        if (exists == true)
                        {
                            string sql2 = $"SELECT ID FROM [dbo].[Book] where InventoryNum=@inventoryNum and IDLibrary={AdminLibraryInfoModel.libraryInfo.Id}";
                            SqlCommand command3 = new SqlCommand(sql2, connection);
                            command3.CommandType = CommandType.Text;
                            command3.Parameters.AddWithValue("@inventoryNum", bookInfo.InventoryNum);
                            ...
                            Response.Redirect("/AdminReturnBookStep2");
                        }
                        else
                        {
